...
press(generalHintPane.component, "\x1b[B");
press(generalHintPane.component, "\x1b[B");
assertContains(plainText(generalHintPane.component), "Use rounded or rectangular editor corners.", "border shape hint should render");
assert.ok(plainRender(generalHintPane.component).some((line) => line.startsWith("╭")), "pane preview should start with rounded corners");
press(generalHintPane.component, "\x1b[C");
press(generalHintPane.component, "\r");
assertLineContainsAll(plainText(generalHintPane.component), ["Border shape", "rectangular"], "General should cycle border shape");
assert.ok(plainRender(generalHintPane.component).some((line) => line.startsWith("┌")), "pane preview should update to rectangular corners immediately");
press(generalHintPane.component, "\x1b[D");
press(generalHintPane.component, "\x1b[B");
press(generalHintPane.component, "\x1b[C");
for (let i = 0; i < 2; i++) press(generalHintPane.component, "\r");
assertLineContainsAll(plainText(generalHintPane.component), ["Min input rows", "1"], "General should cycle from the default height through four to one row");
const oneRowPreview = plainRender(generalHintPane.component);
const previewBodyIndex = findLineIndexContaining(oneRowPreview, "Ask pi to improve the input surface...");
assert.ok(oneRowPreview[previewBodyIndex - 1]?.startsWith("┌"), "one-row preview should retain rectangular top corners");
assert.ok(oneRowPreview[previewBodyIndex + 1]?.startsWith("└"), "pane preview should shrink to one body row immediately");
press(generalHintPane.component, "\x1b[D");
press(generalHintPane.component, "\x1b[B");
